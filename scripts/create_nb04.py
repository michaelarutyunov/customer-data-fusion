...
cells = [
    md(
        ...
        "Rules are derived from `personas.yaml` float params "
        "(`price_sensitivity`, `price_variance_tolerance`, `brand_loyalty`, "
        "`openness_to_new`, `primary_strategy`). "
    ...
    md(
        ...
        "archetype definition (price_sensitivity=0.85, price_variance_tolerance=0.10). "
    ...
    md(
        ...
        "primary attribute. Compensatory and Adaptive are close behind (48%, 42%) — ✓ they "
    ...
    md(
        ...
        "Price Lexicographic shows lowest disruption (25%) — ✓ brand_loyalty=0.25, price is the "
        "primary cue. Brand Heuristic defects to `no_preferred_alternative` — ✓ this archetype "
        "has no fallback heuristic; it is anchored to the specific brand."
    ),
    md(
    ...
    md(
        ...
        "No result contradicts expected persona behaviour. The redistribution rules are "
        "internally consistent with the `personas.yaml` archetype definitions.\n\n"
        "**Limitation:** Redistribution is archetype-uniform — all members of the same archetype "
        "receive the same predicted defect_share regardless of their individual `PersonaConfig` "
        "values. Option B (bead `sei`) addresses this by re-running the trace generator under "
        "modified choice set conditions, enabling individual-level predictions."
    ),
]

nb["cells"] = cells
path = Path("notebooks/04_counterfactual_tests.ipynb")
