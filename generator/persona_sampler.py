...
def sample_persona(
    ...
    transactions = _build_transactions(raw["transactions"], rng)
    psychographic = _build_psychographic(raw["psychographic"], rng)
    narrative = _build_narrative(raw["narrative"])
...
def _build_psychographic(raw: dict, rng: np.random.Generator) -> PsychographicParams:
    return PsychographicParams(
        involvement_score=_noisy(rng, raw["involvement_score"]),
        maximiser_score=_noisy(rng, raw["maximiser_score"]),
        risk_tolerance=_noisy(rng, raw["risk_tolerance"]),
        price_consciousness=PriceConsciousness(raw["price_consciousness"].lower()),
        openness_to_new=_noisy(rng, raw["openness_to_new"]),
    )


def _build_narrative(raw: dict) -> NarrativeParams:
    ...
    return NarrativeParams(
        ...
        decision_style_description=raw["decision_style_description"].strip(),
