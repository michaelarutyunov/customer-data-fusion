...
**Schema-update epic (complete — branch `feature/1it-thread-participant-id`).** Extended to 6 modalities (clickstream + campaign) with variable-modality late fusion. All six beads closed: `1it` (participant_id threading + `--skip-narratives` fix), `33x` (campaign encoder, recovery 0.71), `syu`/`fso` (clickstream encoder + archetype-keying, recovery 0.23→0.52), `hcx` (variable-modality loader + `assert n_modalities` guard), `2io` (full 6-modality run). **6-modality result:** archetype recovery **98.0%**, dropout-view recall@1 **82.5%** (improved over the 4-modality prototype's 70.4%), PersonaConfig R² **0.66** (below the 0.79 gate — an identity-vs-linearity tradeoff). Open follow-ups: `yy7` (participant-duplication bug), `7t6` (config_probe/retrieval variable-modality), `fkx` (R² investigation). See `.claude/context/modality-expansion.md`, `.claude/context/fusion-architecture.md`, and `docs/post-mortems/schema-update-postmortem.md`.