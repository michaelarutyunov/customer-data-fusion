...
class TestGenerateNarrativeDeepSeek:
    @patch.dict(
        "os.environ",
        {"DEEPSEEK_API_KEY": "test-key", "ANTHROPIC_API_KEY": ""},
        clear=False,
    )
    @patch(
        "generator.text_generator._call_deepseek",
        return_value=(MOCK_TEXT, "deepseek-chat"),
    )
...
class TestGenerateNarrativeAnthropicFallback:
    @patch(
        "generator.text_generator._llm_generate",
        return_value=(MOCK_TEXT, "claude-sonnet-4-6"),
    )
    ...
    @patch.dict(
        "os.environ", {"DEEPSEEK_API_KEY": "", "ANTHROPIC_API_KEY": ""}, clear=False
    )
...
class TestGenerateNarrativesBatch:
    ...
    def test_batch_returns_correct_length(self, mock_call):
        ...
        configs = [
            sample_persona(aid, random_seed=i)
            for i, aid in enumerate(["price_lex", "compensatory", "satisficer"])
        ]
    ...
    def test_batch_personas_have_correct_ids(self, mock_call):
        ...
        configs = [
            sample_persona(aid, random_seed=i) for i, aid in enumerate(archetypes)
        ]
        results = generate_narratives_batch(configs)
